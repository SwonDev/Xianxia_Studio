...
pub struct GenerateRequest {
    ...
    /// v0.6.0 — opt-in for LTX-2.3 real video engine (triple-gated).
    /// Default None → false → always HyperFrames (byte-identical default).
    /// Honored only when hardware capability != None AND LTX models are
    /// installed. On any failure falls back to HyperFrames per beat.
...
/// v0.6.0 — Check whether the canonical LTX-2.3 key model files are present
/// on disk. Checks BOTH tier sets (Gguf + Full) because capability resolution
/// already selects the right tier — if ANY tier's key files are present AND
/// the capability matches, we're good. We check the sentinel file for each
/// tier independently so a partial install is treated as "not installed".
///
/// Key files per tier (from docs/superpowers/ltx23-pinned-facts.md):
///   Gguf: diffusion_models/ltx-2.3-22b-dev-Q4_K_M.gguf
///         vae/ltx-2.3-22b-dev_video_vae.safetensors
///         text_encoders/comfy_gemma_3_12B_it.safetensors
///   Full: checkpoints/ltx-2.3-22b-dev-fp8.safetensors
///         text_encoders/comfy_gemma_3_12B_it.safetensors
///
...
pub(crate) fn ltx_models_installed() -> bool {
    ...
    match cap {
        crate::hardware::LtxCapability::Gguf => {
            models.join("diffusion_models").join("ltx-2.3-22b-dev-Q4_K_M.gguf").is_file()
                && models.join("vae").join("ltx-2.3-22b-dev_video_vae.safetensors").is_file()
                && models.join("text_encoders").join("comfy_gemma_3_12B_it.safetensors").is_file()
                && models.join("checkpoints").join("ltx-2.3-22b-dev_embeddings_connectors.safetensors").is_file()
        }
        crate::hardware::LtxCapability::Full => {
            models.join("checkpoints").join("ltx-2.3-22b-dev-fp8.safetensors").is_file()
                && models.join("text_encoders").join("comfy_gemma_3_12B_it.safetensors").is_file()
        }
        crate::hardware::LtxCapability::None => false,
    }
}
...
async fn run(
    ...
    // ─── v0.6.0 — Resolve video engine (triple-gate) ─────────────────
    // DEFAULT is ALWAYS "hyperframes" — byte-identical to pre-v0.6.0.
    // "ltx" requires ALL THREE gates: hardware capable (≥24 GB VRAM) AND
    // LTX models present on disk AND user explicitly opted in via the UI.
    // The user's 8 GB dev machine resolves to LtxCapability::None ALWAYS
    // (proven impossible in hardware.rs tests) so engine is always
    // "hyperframes" there. Nothing downstream changes when engine=="hyperframes".
    let video_engine: &str = {
        let cap = crate::hardware::ltx_video_capability();
        let models_ok = ltx_models_installed();
        let opted_in = req.use_ltx_video.unwrap_or(false);
    ...
    tracing::info!(engine = video_engine, "v0.6.0 video engine resolved");
    ...
    // ─── Phase 4c: LTX-2.3 real video clips (v0.6.0, opt-in) ────────
    // When video_engine == "ltx" we animate each keyframe still with LTX-2.3
    // img2video. This replaces DepthFlow's per-beat clip with a real video
    // clip generated by the LTX ComfyUI workflow. The result is stored in
    // beats[i]["clip_path"] using the SAME key DepthFlow uses, so the
    // unchanged try_hyperframes_render / /render/narrative path in Phase 6
    // picks it up automatically (it already handles clip_path per beat).
    //
    // VRAM choreography: unload(depth) was already called above. We call
    // ensure_comfyui_vram before each beat to respect the project's GPU-only
    // rule (no CPU offload). Between beats we do NOT unload ComfyUI because
    // it stays warm with the LTX model loaded.
    //
    // Resume: each beat's LTX clip is persisted as part of the beats array
    // in phase 4 (same JSON blob). The phase-4 resume guard at the top of
    // Phase 4 already checks both "path" (still) existence. We extend it:
    // when engine==ltx, skip the whole 4c block if every beat already has a
    // non-empty "ltx_clip_path" on disk (engine-namespaced key, never mixed
    // with DepthFlow's "clip_path" from a prior hyperframes run).
    //
    // Fallback: any per-beat LTX failure → tracing::warn + the beat keeps its
    // still image path (clip_path stays absent/unchanged) → Phase 6 falls back
    // to Ken Burns for that beat. HyperFrames handles mixed timelines fine.
    //
    // Default path (video_engine != "ltx"): this entire block is skipped.
    // beats[] is untouched. Phase 6 is byte-identical to pre-v0.6.0.
    if video_engine == "ltx" && !beats.is_empty() {
        emit(app, pid, 4, "running", 85.0, "Animando keyframes con LTX-2.3 (vídeo real)…");
        let cap = crate::hardware::ltx_video_capability();
        let template = if cap == crate::hardware::LtxCapability::Full { "full" } else { "gguf" };
        let ltx_dir = format!("{}/ltx-clips", out_dir);
        let _ = std::fs::create_dir_all(&ltx_dir);
        let total_beats = beats.len();

        // Check if ALL beats already have a valid LTX clip from a prior run.
        // We use "ltx_clip_path" (engine-namespaced) to avoid mixing with
        // DepthFlow's "clip_path". On resume, if ALL are present and valid,
        // we skip generation and promote them to the live "clip_path" key.
        let all_ltx_done = beats.iter().all(|b| {
            b.get("ltx_clip_path")
                .and_then(|v| v.as_str())
        ...
        if all_ltx_done {
            tracing::info!(project = %pid, beats = total_beats,
                "phase 4c: resuming — reusing all {} persisted LTX clips", total_beats);
            for b in beats.iter_mut() {
                if let Some(p) = b.get("ltx_clip_path").and_then(|v| v.as_str()).map(|s| s.to_string()) {
                    b["clip_path"] = serde_json::json!(p);
                }
            }
            emit(app, pid, 4, "done", 100.0, "Clips LTX (reanudados — ya completados)");
        } else {
            // LTX minimum VRAM: after depth is unloaded, we need ComfyUI +
            // LTX model to fit. Conservative floor matching the existing Gguf
            // threshold from pinned-facts (≥24 GB available on capable HW).
            // We use the existing THUMB_MIN_VRAM_GB constant as a floor check
            // sanity guard before the first beat; if VRAM is somehow below
            // even this low bar, skip the entire LTX phase gracefully.
            const LTX_MIN_VRAM_GB: f64 = 20.0; // conservative: Gguf ≥24 GB means ≥20 free after OS
            let pre_vram = ensure_comfyui_vram(app, &client, LTX_MIN_VRAM_GB).await;
            if pre_vram < LTX_MIN_VRAM_GB {
                tracing::warn!(
                    free_gb = pre_vram,
                    target_gb = LTX_MIN_VRAM_GB,
                    "phase 4c: VRAM gate failed before LTX — skipping all beats (fallback HyperFrames)"
                );
                emit(app, pid, 4, "done", 100.0, "LTX skip: VRAM insuficiente — usando HyperFrames");
            } else {
                let mut ltx_attached = 0usize;
                let (ltx_w, ltx_h) = if req.vertical { (768_u32, 1344_u32) } else { (1344_u32, 768_u32) };
                for i in 0..total_beats {
                    let init_image = beats[i]["path"].as_str().unwrap_or("").to_string();
                    let beat_prompt = beats[i].get("prompt")
                        .and_then(|v| v.as_str())
                        .unwrap_or("cinematic landscape")
                        .to_string();
                    let beat_seconds = beats[i]["duration"].as_f64().unwrap_or(4.0)
                        .clamp(2.0, 12.0);

                    if init_image.is_empty() || !std::path::Path::new(&init_image).exists() {
                        tracing::warn!(beat = i, "LTX beat {}: missing init_image — skipping", i);
                        continue;
                    }

                    // Check if this specific beat already has a valid LTX clip.
                    let existing = beats[i].get("ltx_clip_path")
                        .and_then(|v| v.as_str())
                        .map(|p| !p.is_empty() && std::path::Path::new(p).exists())
                        .unwrap_or(false);
                    if existing {
                        let cached_path = beats[i]["ltx_clip_path"].as_str().unwrap_or("").to_string();
                        beats[i]["clip_path"] = serde_json::json!(&cached_path);
                        ltx_attached += 1;
                        tracing::info!(beat = i, "phase 4c beat {}: reusing cached LTX clip", i);
                        continue;
                    }
                    ...
                    match ltx_result {
                        Ok(resp) if resp.status().is_success() => {
                            match resp.json::<serde_json::Value>().await {
                                Ok(json) => {
                                    if let Some(out) = json["out_path"].as_str() {
                                        if !out.is_empty()
                                            && std::fs::metadata(out)
                                                .map(|m| m.len() > 1024)
                                                .unwrap_or(false)
                                        {
                                            // Persist under engine-namespaced key to avoid
                                            // mixing with DepthFlow artifacts. Also set
                                            // "clip_path" so Phase 6 picks it up.
                                            beats[i]["ltx_clip_path"] = serde_json::json!(out);
                                            beats[i]["clip_path"] = serde_json::json!(out);
                                            ltx_attached += 1;
                                            tracing::info!(beat = i, out, "LTX clip generated");
                                        } else {
                                            tracing::warn!(beat = i,
                                                "LTX beat {}: out_path empty/small — fallback HyperFrames", i);
                                        }
                                    }
                                }
                                Err(e) => {
                                    tracing::warn!(beat = i, error = %e,
                                        "LTX beat {}: JSON decode failed — fallback HyperFrames", i);
                                }
                            }
                        }
                        Ok(resp) => {
                            let status = resp.status();
                            tracing::warn!(beat = i, %status,
                                "LTX beat {}: HTTP {} — fallback HyperFrames", i, status);
                        }
                        Err(e) => {
                            tracing::warn!(beat = i, error = %e,
                                "LTX beat {}: request failed — fallback HyperFrames", i);
                        }
                    }

                    let pct = 85.0 + ((i + 1) as f64 / total_beats as f64) * 12.0;
                    emit(app, pid, 4, "running", pct,
                        &format!("LTX-2.3 clip {}/{}", i + 1, total_beats));
                } // end per-beat loop

                // Re-persist the updated beats (now with ltx_clip_path/clip_path keys).
                // Uses the same phase-4 slot — the resume guard already exists for it.
                if ltx_attached > 0 {
                    persist_step(pool, pid, 4, "done",
                        &serde_json::json!({"beats": beats})).await?;
                }
                tracing::info!(
                    ltx_attached, total = total_beats,
                    "phase 4c: {}/{} beats animated with LTX-2.3; rest fallback to HyperFrames",
                    ltx_attached, total_beats
                );
                if ltx_attached == total_beats {
                    emit(app, pid, 4, "done", 100.0, "LTX-2.3 vídeo real listo (todos los beats)");
                } else if ltx_attached > 0 {
                    emit(app, pid, 4, "done", 100.0,
                        &format!("LTX-2.3 parcial: {}/{} beats; rest KenBurns", ltx_attached, total_beats));
                } else {
                    emit(app, pid, 4, "done", 100.0, "LTX-2.3 no produjo clips — usando HyperFrames");
                }
            }
        }
        // Unload LTX/ComfyUI models before Music phase.
        unload(&client, "comfyui").await;
    }
    // (when video_engine != "ltx" this block is entirely skipped — default path byte-identical)
...
mod tests {
    ...
    fn deserializes_minimal_payload_from_frontend() {
        ...
        assert!(!req.vertical);
        // v0.6.0: use_ltx_video absent → None (default false; default path = hyperframes)
        assert_eq!(req.use_ltx_video, None);
    }

    #[test]
    fn use_ltx_video_field_is_optional_and_defaults_to_none() {
        // Existing requests that omit use_ltx_video must deserialize unchanged.
        let json_no_ltx = r#"{"topic":"x","languages":["en"],"target_minutes":5,"experimental_llm":false}"#;
        let req: GenerateRequest = serde_json::from_str(json_no_ltx).expect("minimal payload");
        assert_eq!(req.use_ltx_video, None,
            "absent use_ltx_video must deserialize to None (default path = hyperframes)");

        // Explicit true must round-trip.
        let json_ltx_true = r#"{"topic":"y","languages":["en"],"target_minutes":5,"experimental_llm":false,"use_ltx_video":true}"#;
        let req2: GenerateRequest = serde_json::from_str(json_ltx_true).expect("ltx true payload");
        assert_eq!(req2.use_ltx_video, Some(true));

        // Explicit false must round-trip.
        let json_ltx_false = r#"{"topic":"z","languages":["en"],"target_minutes":5,"experimental_llm":false,"use_ltx_video":false}"#;
        let req3: GenerateRequest = serde_json::from_str(json_ltx_false).expect("ltx false payload");
        assert_eq!(req3.use_ltx_video, Some(false));
    }
